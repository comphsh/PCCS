...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            out_main = net(input)
            if isinstance(out_main , dict):
                out_main = out_main["seg"]
            # if len(out_main)>1:
            ...
            out_main = F.interpolate(out_main , size=label.shape[-2:] , mode='bilinear' , align_corners=True)
...
def test_single_volume_output_dir_slices(case, net, test_save_path, FLAGS):
    h5f = h5py.File(FLAGS.root_path + "/data/{}.h5".format(case), 'r')
    image = h5f['image'][:]
    label = h5f['label'][:]
    # label[label==1]=0
    ...
    for ind in range(image.shape[0]-1 , -1 , -1):
        slice = image[ind, :, :]
        slice_name = case + "_slice_"+str(ind + 1)  # patient001_frame01_slice_1.h5
        ...
        with torch.no_grad():
            ...
            if isinstance(out_main, dict):
                if 'seg' in out_main:
                    out_main = out_main["seg"]
                elif 'pred' in out_main:
                    out_main = out_main['pred']
                elif 'out' in out_main:
                    out_main = out_main['out']

            elif isinstance(out_main , tuple):
                out_main = out_main[0]
            # if len(out_main)>1:
            #     out_main=out_main[0]#torch.softmax(out_main[0], dim=1)+ torch.softmax(out_main[1], dim=1)+torch.softmax(out_main[3], dim=1)
            #     #out_main=out_main[4].transpose(0,1).reshape(4,256,256).unsqueeze(0)
            ...
            prediction[ind] = pred
            # print("{} {} {} shape{} {} {}".format(np.unique(image) , np.unique(label) , np.unique(pred) ,
            #                                       np.array(image).shape , np.array(label).shape , np.array(pred).shape))  #0~1 ,0 1 2 3
            # save_result_hXw(img * 255., label , pred, test_save_path, img_name ,FLAGS.num_classes,opacity=0.5)
            # 保存原图
            target_image, target_mask , target_pred = crop_by_ratio(image[ind], label[ind] , pred , target_ratio=0.1, resize=[224, 224]) #224 只是方便查看
            save_result_hXw(target_image * 255 , target_mask , target_pred , test_save_path , slice_name , FLAGS.num_classes , opacity=0.4 )
...
def save_result_hXw(img, label ,pred , file_path,
                  img_name, num_classes ,opacity=0.5, palette=[[0, 0, 0], color44 , color7 , color9 ] , bound_palatee=[[0,0,0] , color66 , color66 , color66]):
    ...
    img_labelbound_pred = img_3D * (1 - opacity) + (0.5*color_label_bound + 0.5*color_pred) * opacity  # img与maskbound和predmask合并
...
def test_single_2D_2class(sample_dict , net, test_save_path, FLAGS , cfg , resize_size=[] , metric_list = [], metric_name=None , calculated_num=0 , divide_name='' ):
    net.eval()
    # 测试不需要增强
    ...
    input , label , img_name = sample_dict['image'].float().cuda() ,  sample_dict['label'] , sample_dict['img_name'][0]
    # print("input ={}  label={}  imgname {}".format( input.shape , label.shape   , img_name))
    # and int(img_name[1]) <=3
    if divide_name in img_name   : #恶性 malignant 42 ， 良性 70 benign
        pass #通过
    else:
        return

    # if divide_name=='L' and 'T1' in img_name:
    #     return


    calculated_num[0] = calculated_num[0] + 1
    # print("canlnum={}".format(calculated_num))

    label = label.squeeze(1).squeeze(0).numpy()
    img = input.mean(dim=1).squeeze().cpu().numpy()
    input = F.interpolate(input.float() , size=resize_size , mode='bilinear' , align_corners=True)


    # if img_name == 'benign (6)':
    #     # show_single_mask_(img * 255 ,label, FLAGS.num_classes)
    #     show_single_mask_(srimg.squeeze()[:,:,0], srlabel.squeeze(), FLAGS.num_classes)
    #     exit()
    # else:
    #     return metric_list
    assert resize_size[0] == input.shape[-2:][0] and resize_size[1] == input.shape[-2:][1] , print("resize={}  input={}".format(resize_size , input.shape))
    ...
    #     show_uncertainty(unet_uncertainty_map , img_name)
    # else:
    #     return metric_list

    assert label.shape == pred.shape , print(label.shape , pred.shape)
    for cls in range(1, FLAGS.num_classes):  # 一个batch 同类算一次
        val_i = cal_2d_by_metricname(pred == cls, label == cls , metric_name)  # 这里只算平均，，每个类的值，到了每一类都在这了
        metric_list[cls - 1] += np.array(val_i)

    ### -----------保存img ,label , pred , img+pred
    # opacity=0.0 对应img  opacity=1 对应mask opacity=0.5对应一半
    # save_result_hXw(img * 255., label , pred, test_save_path, img_name ,FLAGS.num_classes,opacity=0.5)


def Inference_tar_gz(output_path , pth_name , FLAGS , cfg=None):
    with open(FLAGS.root_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0] for item in image_list])

    test_save_path = "{}/{}_{}_{}_predictions_tar_gz/".format(output_path , FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = net_factory(net_type=FLAGS.model, in_chns=1, class_num=FLAGS.num_classes , cfg=cfg)
...
def Inference_dir_slices(output_path, pth_name, FLAGS , cfg=None , methods_name=''):
    with open(FLAGS.root_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0] for item in image_list])

    test_save_path = "{}/{}_{}_{}_predictions_dir_slices/".format(output_path , FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = net_factory(net_type=FLAGS.model, in_chns=1, class_num=FLAGS.num_classes , cfg=cfg).cuda()
    save_model_path = f"{output_path}/{pth_name}"#os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))#"/home/zxzhang/MC-Net-Main/data/ACDCSASSNetACDCclass3/data/ACDC_SASSNetACDC_7_labeled/unetsdf//iter_6200_dice_0.8938.pth"#os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))#os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))#os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))
    ...
    third_total = 0.0
    for case in tqdm(image_list):
        first_metric, second_metric, third_metric = test_single_volume_output_dir_slices(case, net, test_save_path, FLAGS)
        first_total += np.asarray(first_metric)
        second_total += np.asarray(second_metric)
        third_total += np.asarray(third_metric)
    avg_metric = [first_total / len(image_list), second_total / len(image_list), third_total / len(image_list)]
...
def Inference_2D(output_path , pth_name , FLAGS , metric_name=None ,resize_size=[0,0] , cfg = None , methods_name='' , divide_name=''):
    data_mode = 'test_DM'  #avg test=[0.43155008 0.35353266]  mip_test=[0.42620746 0.34713619]  t1a_test=[0.48093323 0.40090217]   t2_test=[0.38851047 0.31349084] ,3个平均0.43188372

    test_save_path = "{}/{}_{}_{}_predictions_{}/".format(output_path, FLAGS.exp, FLAGS.labeled_num, FLAGS.model , data_mode)
    print("save path {}".format(test_save_path))
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)  # 递归删除该目录下的所有文件
    os.makedirs(test_save_path)

    if 'breast' in FLAGS.exp:
        data_loader = get_breast_data_loader(args=FLAGS, cfg=cfg, data_mode=data_mode)
    elif 'BUSI' in FLAGS.exp:
    ...
    metric_list = np.zeros((FLAGS.num_classes - 1, len(metric_name)))  #两个度量方式， DICE IOU
    calculated_num = [0]
    for i , sample_dict in tqdm(enumerate(data_loader)):
        test_single_2D_2class(sample_dict,  net, test_save_path, FLAGS ,cfg, resize_size , metric_list , metric_name  , calculated_num , divide_name )  #我懂了，，python一切都是对象，一切都是引用！！引用传递，自动改变
        # print("i = {} , metric = {} , valmetric = {}".format(i , metric_list , metric_list / (i+1)))


    # metric_list = metric_list / len(data_loader)
    calculated_num = calculated_num[0]
    print("calculated_num={}".format(calculated_num))
    metric_list = metric_list / calculated_num

    avg_metric = np.mean(metric_list , axis=0)
    return metric_list , avg_metric, test_save_path


def Inference_metric(cfg , FLAGS , output_path , pth_name , methods_name , divide_name , divide_result  , loop_mode=None):
    if cfg["dataset"]["dimension"] == "3D": #专指的是ACDC这种，有4类 ,也有两种显示方式，一种方式是变成tar.gz ,一种方式是，变成多个gt img pred img_gtbound_pred
        # metric, test_save_path = Inference_tar_gz(output_path ,pth_name, FLAGS, cfg=cfg)
        metric, test_save_path = Inference_dir_slices(output_path, pth_name, FLAGS , cfg=cfg , methods_name=methods_name)   #只是输出保存的方式不一样。。。
        ...
        metric_name = ["Dice", "Jaccard", 'Recall', 'Precision' ,'Specificity', 'Accuracy']
        metric_list , avg_metric, test_save_path = Inference_2D(output_path,pth_name ,FLAGS , metric_name,resize_size=[FLAGS.patch_size[0],FLAGS.patch_size[1]] , cfg=cfg ,methods_name=methods_name , divide_name=divide_name)
        print("methods_name=={}".format(methods_name))
        print(metric_list)
        print(avg_metric)
...
if __name__ == '__main__':
    ...
    busi_DM_merge = [
        # '/data/heshihuan/program/mapFromWin/BCP-series/BCP-work-007_1/model/breast_busi_DM_merge_full_supervised_1.0_labeled/resunet/resunet_best_loss_model1.pth',
        '/data/heshihuan/program/mapFromWin/BCP-series/BCP-work-007_1/model/breast_busi_DM_merge_semi_0.2_labeled/resunet/iter_model1_13500_dice_0.8593.pth',
        '/data/heshihuan/program/mapFromWin/BCP-series/BCP-work-007_2/model/breast_busi_DM_merge_semi_0.2_labeled/resunet_scp/iter_model1_14000_dice_0.86.pth',
        '/data/heshihuan/program/mapFromWin/BCP-series/BCP-work-007_2/model/breast_busi_DM_merge_semi_0.2_labeled/res_unet_bcp_3_attn/iter_model1_13000_dice_0.8597.pth',
    ]

    # 67.58  58.43  67.49  74.49  87.98  86.28  58.94  48.19  53.69  75.94  86.88  81.98  64.81  55.15  63.06  74.95  87.62  84.90
    # 68.56  59.73  67.56  76.49  89.25  87.18  61.07  49.25  53.49  82.57  91.63  86.49  66.16  56.37  63.05  78.44  90.01  86.96
    #遍历多次取不确定性图

    for pos , methods_output_path in enumerate(busi_DM_merge):

        if 'ACDC' in methods_output_path:
            ...
            FLAGS.root_path = cfg["dataset"]["root_dir"]
            # FLAGS.root_path = r'/data/heshihuan/datasets/medical_firstBreastData/breast_us_1k/breast_1k/large/train_val_test_256X256_7_1_2'
            ...
            print("args", FLAGS)
            Inference_metric(cfg, FLAGS, output_path, pth_name, methods_name, None, None)
        else:
            ...
            for divide_name in divide_list:
                ...
                FLAGS.in_chns = cfg['dataset']['in_channels']
                print("args", FLAGS)
                Inference_metric(cfg, FLAGS, output_path, pth_name, methods_name, divide_name, divide_result )
            print("pth={}".format(methods_output_path))
            print("divide list  = {}".format(divide_list))
            print("methods result {}  ".format(methods_name))
            for val in divide_result:
                print("{:.2f}  ".format(100.0 * val), end='')
            print('\n\n')

        # Inference_TSNE(cfg , FLAGS , output_path , pth_name , layername="decoder1.up2.conv.conv_conv.4")
        # Inference_CAM(cfg, FLAGS, output_path, pth_name, layername="decoder_ppcn.classifier2.3")
