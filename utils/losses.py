...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False):
        ...
        # print("socre={}  target={}".format(inputs.unique(), target.unique()))
        assert len(inputs.shape) == len(target.shape)
        if softmax:
...
class softDiceLoss(nn.Module):
    ...
    def forward(self, inputs, target):
        ...
        class_wise_dice = []
        loss = 0.0
        # acdc:inputs=torch.Size([16, 4, 224, 224])  target=torch.Size([16, 4, 224, 224])
        # inputs=torch.Size([16, 2, 224, 224])  target=torch.Size([16, 2, 224, 224])
        # print("inputs={}  target={}".format(inputs.shape , target.shape))
        for i in range(0, self.n_classes):
            dice = self._dice_loss(inputs[:, i], target[:, i])
            class_wise_dice.append(1.0 - dice.item())
            loss += dice
...
class VAT2d(nn.Module):
    ...
    def __init__(self, xi=10.0, epi=6.0, ip=1, num_classes=0):
