...
def compute_contra_memobank_loss(
    ...
    # print("rep_teacher={}".format(rep_teacher.shape))
    ...
    for i in range(num_segments):
        ...
        # keys=tensor([], device='cuda:0', size=(0, 128)) torch.Size([0, 128])
...
def compute_contra_memobank_loss(
    ...
    # torch.Size([8, 4, 128, 128]) tensor([9.9572e-23, 1.2407e-22, 1.2960e-22,  ..., 1.0000e+00, 1.0000e+00,
    #         1.0000e+00],
    # print(prob_l.shape , prob_l.unique())
    # exit()
    _, prob_indices_l = torch.sort(prob_l, 1, True)  #True 表示从大到小 排序，有标签概率概率 ， 每一条像素的通道进行独立排序 #indices表示原来在哪个通道(类)
    prob_indices_l = prob_indices_l.permute(0, 2, 3, 1)  # (num_labeled, h, w, num_cls)
    ...
    valid_classes = []
    new_keys = []
    for i in range(num_segments):  #计算每一个类下， 每个图像是i类的可靠像素 和 不可靠像素
        low_valid_pixel_seg = low_valid_pixel[:, i]  # select binary mask for i-th class 第i类，要么0 要么1
        high_valid_pixel_seg = high_valid_pixel[:, i]

        prob_seg = prob[:, i, :, :]
        rep_mask_low_entropy = (   prob_seg > current_class_threshold  ) * low_valid_pixel_seg.bool()

        # pro_seg=tensor([1.8766e-15, 2.0835e-15, 2.1406e-15,  ..., 2.0945e-01, 2.0947e-01,2.0951e-01], device='cuda:0') torch.Size([16, 29, 29])

        rep_mask_high_entropy = (  prob_seg < current_class_negative_threshold   ) * high_valid_pixel_seg.bool()

        # high_valid_pixel_seg=47.0  size=torch.Size([16, 4, 29, 29])
        # print("high_valid_pixel_seg={}  size={}".format(torch.sum(high_valid_pixel_seg) , high_valid_pixel.shape))
        # low_valid_pixel_seg=47.0  size=torch.Size([16, 29, 29])
        # print("low_valid_pixel_seg={}  size={}".format(torch.sum(low_valid_pixel_seg), low_valid_pixel_seg.shape))
        ...
        # generate class mask for unlabeled data
        # prob_i_classes = prob_indices_u[rep_mask_high_entropy[num_labeled :]] ，是否有类被i 被识别到较小的 lowrank-- highrank这个排序范围
        class_mask_u = torch.sum(  prob_indices_u[:, :, :, low_rank:high_rank].eq(i), dim=3  ).bool()
        # unlabeled image pixel --> 如果一个像素类被识别成小概率，就极为1

        # generate class mask for labeled data
        # label_l_mask = rep_mask_high_entropy[: num_labeled] * (label_l[:, i] == 0)
        # prob_i_classes = prob_indices_l[label_l_mask] ，， prob_indices_l[:, :, :, :0]表示，像素第1大概率是？类，prob_indices_l[:, :, :, :1] ， 像素的第二大概率是
        class_mask_l = torch.sum( prob_indices_l[:, :, :, :low_rank].eq(i), dim=3 ).bool()

        class_mask = torch.cat(  (class_mask_l * (label_l[:, i] == 0), class_mask_u), dim=0  )  #概率大，但是仍然预测成背景，从预测的第i个类的背景中找出像素

        negative_mask = rep_mask_high_entropy * class_mask  #
        # negative_mask = torch.Size([16, 128, 128])  rep_teacher=torch.Size([16, 128, 128, 128])
        # print("negative_mask = {}  rep_teacher={} ".format(negative_mask.shape  ,   rep_teacher.shape))

        keys = rep_teacher[negative_mask].detach()  #找到mask==1的像素组成向量矩阵 ,这里，rep:bs h w ,chns,,因此可以提取1对应的特征
        # print("negative_mask = {}  rep_teacher={} keys={}".format(negative_mask.shape, rep_teacher.shape , keys.shape))

        # keys=tensor([], device='cuda:0', size=(0, 128)) torch.Size([0, 128])
        # print("keys={}".format(keys.shape))
        # negative_mask=torch.Size([16, 29, 29]) tensor([False], device='cuda:0')
        # print("negative_mask={} {}".format(negative_mask.shape , negative_mask.unique()))
        # rep_mask_high_entropy=torch.Size([16, 29, 29]) tensor([False,  True], device='cuda:0')
        # print("rep_mask_high_entropy={} {}".format(rep_mask_high_entropy.shape, rep_mask_high_entropy.unique()))
        # class_mask=torch.Size([16, 29, 29]) tensor([False,  True], device='cuda:0')
        # print("class_mask={} {}".format(class_mask.shape, class_mask.unique()))
        # print("-----------------------------------------")
        # 第i个 ，membank=[tensor([], size=(0, 128))]  que_ptr=torch.Size([1])  que_size=50000 proto=torch.Size([256, 1, 256])
        new_keys.append(
            dequeue_and_enqueue(
                keys=keys,  #keys 是
                queue=memobank[i],  #第i类的men
                queue_ptr=queue_prtlis[i],
                queue_size=queue_size[i],
            )
        )
