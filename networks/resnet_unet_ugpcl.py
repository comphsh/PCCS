...
class UNetTF(BaseModel2D):
    ...
    def __init__(
        ...
        self.patches_embed = nn.ModuleList([
            # PatchEmbed(img_size=56, patch_size=1, in_chans=256, embed_dim=96, norm_layer=norm_layer),
            # PatchEmbed(img_size=28, patch_size=1, in_chans=512, embed_dim=192, norm_layer=norm_layer),
            # PatchEmbed(img_size=14, patch_size=1, in_chans=1024, embed_dim=384, norm_layer=norm_layer),
            # PatchEmbed(img_size=7, patch_size=1, in_chans=2048, embed_dim=768, norm_layer=norm_layer),

            PatchEmbed(img_size=56, patch_size=1, in_chans=64, embed_dim=96, norm_layer=norm_layer),
            PatchEmbed(img_size=28, patch_size=1, in_chans=128, embed_dim=192, norm_layer=norm_layer),
            PatchEmbed(img_size=14, patch_size=1, in_chans=256, embed_dim=384, norm_layer=norm_layer),
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(
                img_size // patch_size, img_size // patch_size), dim_scale=4, dim=embed_dim)
            self.output = nn.Conv2d(
                in_channels=embed_dim, out_channels=classes, kernel_size=1, bias=False)
        self.cls_head = ClassificationHead(in_channels=self.encoder.out_channels[-1], classes=4) if cls else None
    ...
    def forward(self, x, device):
        ...
        seg_tf = self.up_x4(p)
        seg = self.seg_head(self.decoder(*features))
    ...
    def inference_tf(self, x, device='cuda', **kwargs):
        features = self.encoder(x)
        patches = []
        for i, f in enumerate(features[2:]):
            p_embed = self.patches_embed[i](f)
