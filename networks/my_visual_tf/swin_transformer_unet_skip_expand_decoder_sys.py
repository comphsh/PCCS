...
#         if pooling not in ("max", "avg"):  raise ValueError("Pooling should be one of ('max', 'avg'), got {}.".format(pooling))
...
class ClassificationHead(nn.Module):
    ...
    def __init__(self, in_channels, classes, pooling="avg", dropout=0.2, activation=None):
        ...
        self.sq = nn.Sequential(
            ...
            nn.Conv2d(classes, classes, kernel_size=1),
            nn.Flatten(),
        )
...
class EmbeddingHead(nn.Module):
    def __init__(self, dim_in=64, embed_dim=256, embed='convmlp'):
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=1, num_classes=1000,
        ...
        if self.final_upsample == "expand_first":
            ...
            # print("img pat {} {}".format(img_size  , patch_size))
        ...
        # lowmap=torch.Size([8, 196, 384]) --> [8,256]
        self.cls_head = ClassificationHead(in_channels=384, classes=256) #最底层
        self.embedding_head = EmbeddingHead(dim_in=192 , embed_dim=seg_out_dim) #倒数第二层

        self.fuse_attn1 = MACBlock(seg_out_dim + 3, seg_out_dim, 16)  # 融入img
        self.fuse_attn2 = MACBlock(seg_out_dim + 1, seg_out_dim, 16)
    ...
    def initialize(self):
        ...
        kaiming_normal_init_weight(self.embedding_head)
        kaiming_normal_init_weight(self.fuse_attn1)
        kaiming_normal_init_weight(self.fuse_attn2)
    ...
    def forward_up_features(self, x, x_downsample , select=None):
        low_map = None
        embedding_map = None
        ...
        return x , low_map , embedding_map
    ...
    def media_up_x(self , x , patch_resolution , dim=36 , dim_scale=4):
        H, W = patch_resolution
        B, L, C = x.shape
        print("xx {}  {}".format(x.shape , patch_resolution))
        this_up = FinalPatchExpand_X4(patch_resolution , dim=dim , dim_scale=4).cuda()
        assert L == H * W, "input features has wrong size"
    ...
    def forward(self, x):
        ...
        selected_embed = 1
        x , low_map , embedding_map = self.forward_up_features(x, x_downsample , select=selected_embed)
        low_map = low_map.permute(0 , 2 ,1)
        bs , chns, L = low_map.shape
        low_map = low_map.view(bs , chns , self.window_size *2 , self.window_size *2)  #low_map=torch.Size([4, 384, 14, 14])  embedding_map=torch.Size([4, 784, 192])


        embedding_map = embedding_map.permute(0 , 2 ,1)
        bs , chns , L = embedding_map.shape
        hw = self.window_size*(2**(selected_embed + 1))
        embedding_map = embedding_map.contiguous().view(bs , chns , hw , hw)

        cls_map = self.cls_head(low_map)    # low_map=torch.Size([4, 384, 14, 14])
        embedding_map = self.embedding_head(embedding_map)

        # print("low_map={}  embedding_map={}".format(cls_map.shape,   embedding_map.shape))    #low_map=torch.Size([4, 256])  embedding_map=torch.Size([4, 128, 28, 28])

        x = self.up_x4(x)

        return {'seg':x , 'cls_map':cls_map , 'embedding':embedding_map}
