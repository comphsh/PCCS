...
class ToTensor:  #太恶心人了，为默认情况下，该操作会将数据归一化到 [0, 1] 范围 可能会破坏label的离散值
    """
    ...
    def __call__(self, sample):
        ...
        # sample = {'image': F.to_tensor(sample['image']),   #对图像归一化？？，我去！
        #         'label': F.to_tensor(sample['label'])}   #可能会破坏label的离散值 并且自动增加一个维度
        sample = {'image': F.to_tensor(sample['image']),  #PIL转tensor，自动归一化 并且维度变换
...
class Resize(T.Resize):
    ...
    def forward(self, sample):
        sample['image'] = F.resize(sample['image'], self.size, self.interpolation) #torch.vision非常奇葩，其输入必须是[256，256，chnns]
