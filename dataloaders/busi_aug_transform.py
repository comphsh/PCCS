...
class BUSIDataSets(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.array(img)  #这里不归一，留待F.to_tensor归一
        mask = np.array(mask)
        # 放缩到0 和 1
        ...
        if self.split == "train":
            ...
            sample = {'image': Image.fromarray(img), 'label': Image.fromarray(mask)}
...
def get_busi_data_loader(args , cfg=None):
    ...
    db_train = BUSIDataSets(base_dir=args.root_path, split="train", num=None,  transform=cfg['dataset']['kwargs']['train_transforms'], in_chns=args.in_chns)

    db_val = BUSIDataSets(base_dir=args.root_path, split="val" , transform=cfg['dataset']['kwargs']['train_transforms'], in_chns=args.in_chns)
